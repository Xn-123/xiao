      ...
      <div class="py-container">
        ...
        <div class="bread">
          ...
          <ul class="fl sui-tag">
            ...
            <!-- 关键字面包屑 -->
            <li class="with-x" v-if="searchParams.keyCord">{{searchParams.keyCord}}
              <i @click="removekeyCord">×</i>
            </li>
            <!-- 品牌面包屑  -->
            ...
            <!-- 平台参数面包屑  -->
            <li class="with-x" v-for="(prop,index) in searchParams.props" :key="index">{{searchParams.props[index].split(":")[1]}}
...
export default {
  ...
  data() {
    return {
      ...
      searchParams:{
        ...
        // 关键字
        "keyCord": "",
        // 排序 默认排序 (1:desc)  1综合 2价格 asc desc  
        "order": "1:desc",
        // 分页器 当前第几页
        "pageNo": 1,
        // 代表搜索结果最终展现个数
  ...
  methods:{
    ...
    // 移除关键字面包屑
    removekeyCord(){
       this.searchParams.keyCord=undefined;
       // 重新请求数据
       this.getData();
       // 事件总线触发 清除输入框中的 关键字  （Header组件绑定 Search组件触发）
       this.$bus.$emit('clear'); 
       // 跳转路由
    ...
    changeOrder(flag){
      ...
      let orderSlot =  this.searchParams.order.split(":")[1]
      if(flag==orderFlag){
        order = `${orderFlag}:${orderSlot == 'desc'?'asc':'desc'}`
      }else{
        order = `${flag}:${orderSlot == 'desc'?'desc':'asc'}`
      }
      // 修改参数
